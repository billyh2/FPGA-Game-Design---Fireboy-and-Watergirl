...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE writeData[2];  // Array for the register + value
	int status;

	    // select the MAX3421E
	status = XSpi_SetSlaveSelect(&SpiInstance, 1); // '1' corresponds to the MAX3421E
	if (status != XST_SUCCESS) {
	    xil_printf("SPI Select error: %d\n", status);
	    return;
	}

	//iniitialize the data
	writeData[0] = reg + 2;  // Register + 2, as per the protocol
	writeData[1] = val;      // The value to write to the register

	    // write the data to SPI
	status = XSpi_Transfer(&SpiInstance, writeData, NULL, 2); // Send two bytes
	if (status != XST_SUCCESS) {
	    xil_printf("SPI write error: %d\n", status);
	}

	//deselect after write
	XSpi_SetSlaveSelect(&SpiInstance, 0);  // Deselect by clearing slave select
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return (data + nbytes);
    BYTE writeBuffer[nbytes + 1];  // buffer to hold the register and data
    int status;
    ...
    // Prepare the buffer to send via SPI
    writeBuffer[0] = reg + 2;
    ...
    status = XSpi_Transfer(&SpiInstance, writeBuffer, NULL, nbytes + 1);  // Send the register and the data
    if (status != XST_SUCCESS) {
        xil_printf("SPI write error: %d\n", status);
        return NULL;
    }

    // deselect the MAX3421E
    XSpi_SetSlaveSelect(&SpiInstance, 0);

    // return pointer to the memory position after the last written byte
    return data + nbytes;
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	status = XSpi_Transfer(&SpiInstance, readBuffer, readBuffer, 2); //read and write through readBuffer
	if(status != XST_SUCCESS) {
		xil_printf("SPI write error: %d\n", status);
	}
	...
	return readBuffer[1]; //return the data
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return (data + nbytes);
    BYTE writeBuffer[1];  // Buffer to hold the register address
    BYTE readBuffer[nbytes];  // Buffer to hold the data read from the device
    int status;
    ...
    // write the register address and read the nbytes of data
    status = XSpi_Transfer(&SpiInstance, writeBuffer, readBuffer, nbytes);
    if (status != XST_SUCCESS) {
        xil_printf("SPI read error: %d\n", status);
        return NULL;  // Return NULL to indicate failure
    }

    // Copy the data from readBuffer to the user-provided data buffer
    ...
    // return the pointer to the memory position after the last read
    return data + nbytes;
