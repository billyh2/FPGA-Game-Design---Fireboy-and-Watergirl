...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	writeData[1] = val;      // The value to write to the register
	    ...
	    // select the MAX3421E
	XSpi_SetSlaveSelect(&SpiInstance, 1); // '1' corresponds to the MAX3421E
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return (data + nbytes);
    BYTE writeBuffer[nbytes + 1];  // Buffer to hold the register address
    BYTE readBuffer[nbytes + 1];  // Buffer to hold the data read from the device
    int status;
